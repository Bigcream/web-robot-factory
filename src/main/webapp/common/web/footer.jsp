<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	
<!-- Footer -->
<section id="footer-widget" class="footer-widget">
	<div class="container header-bg">
		<div class="row">
			<div class="col-sm-3">
				<h3>Our Popular Services</h3>
				<ul>
					<li><a href="<c:url value='/Space-robot?page=1'/>">Space Robot</a></li>
					<li><a href="#">Lego Robot</a></li>
					<li><a href="<c:url value='/Toy-for-kid?page=1'/>">Toy for Robot</a></li>
					<li><a href="#">Industry Robot</a></li>
					<li><a href="<c:url value='/Sport-robot?page=1'/>">Sports Robot</a></li>
				</ul>
			</div>
			<div class="col-sm-3">
				<h3>Important Link</h3>
				<ul>
					<li><a href="#">Lorem</a></li>
					<li><a href="#">Ipsum</a></li>
					<li><a href="#">Dolar</a></li>
					<li><a href="#">Set amet</a></li>
					<li><a href="#">Iodiet lorem</a></li>
				</ul>
			</div>
			<div class="col-sm-3">
				<h3>Our Latest Services</h3>
				<ul>
					<li><a href="#">Edu Robot</a></li>
					<li><a href="#">Low Robot</a></li>
					<li><a href="#">Mega Robot</a></li>
					<li><a href="#">Industry Robot</a></li>
					<li><a href="#">Sports Robot</a></li>
				</ul>
			</div>
			<div class="col-sm-3">
				<h3>Our Services</h3>
				<div class="widget-img-box">
					<a class="test-popup-link" href="<c:url value='/template/web/assets/images/widget-1.png'/>">
					
						<img class="widget-img" src="<c:url value='/template/web/assets/images/widget-1.png'/>"
						alt="widget">
					</a> <a class="test-popup-link" href="<c:url value='/template/web/assets/images/widget-2.png'/>">
					
						<img class="widget-img" src="<c:url value='/template/web/assets/images/widget-2.png'/>"
						alt="widget">
					</a>
				</div>
			</div>
		</div>
	</div>
</section>
<footer class="footer text-center">
	<h3>
		&copy; Theme by <a href="https://themewagon.com/">Themewagon</a>
	</h3>
</footer>